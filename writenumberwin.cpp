...
#include <vector>
using namespace std;
/*IMPLAMANTATION OF THE ALGORITHM

Implementation of the algorithm of ten random trees.
The algorithm draws ten digits and sets them, then takes the next random
ten digits and sets them with the previous one, then shifts
one position and sets with previus, then takes ten times, then takes
another 10 digits and does the same 100 times and so on..
*/
...
do{
    ...
    if ((P[dl]>=1)&&(dl>1)){ SetConsoleTextAttribute( hOut, 0x007 );}//change    the colour for new counted Real numbers
    std::cout << "\n A(";
    for(i=dl;i>=1;i--){
